...
async function cmdOnboard(nonInteractive = false) {
  console.log(`\n${BOLD}${CYN}╔══════════════════════════════════════╗${R}`);
  ...
  // If in non-interactive mode and config already exists, skip onboarding
  if (nonInteractive) {
    const configPath = path.join(AURA_DIR, 'config.yaml');
    if (fs.existsSync(configPath)) {
      console.log(info('Configuration already exists. Skipping onboarding.'));
      return;
    }
  }

  // Rebuild native addons for this machine's Node.js ABI.
  ...
  const nativeModules = ['better-sqlite3', 'mdns'];
  const failedModules = [];

  for (const mod of nativeModules) {
    const rebuildResult = spawnSync('npm', ['rebuild', mod], {
      cwd: __dirname, stdio: 'inherit', shell: true,
    });
    if (rebuildResult.status !== 0) {
      failedModules.push(mod);
      console.log(warn(`${mod} rebuild failed — this module may not work.`));
    }
  }

  if (failedModules.length > 0) {
    console.log(warn(`Some native modules failed to rebuild: ${failedModules.join(', ')}`));
    console.log(warn('You may need build tools (python3, make, g++).'));
    console.log(warn('On Ubuntu/Debian: sudo apt install -y python3 make g++'));
    console.log(warn('On macOS: Install Xcode Command Line Tools: xcode-select --install'));
    console.log(warn('On Windows: Install windows-build-tools or Visual Studio Build Tools'));
  } else {
  ...
  try {
    ...
    if (llm === 'claude') {
      ...
      try {
        ...
        console.log(warn(`  Ollama not reachable at ${process.env.OLLAMA_BASE_URL || 'localhost:11434'} — start it first.`));
      }
    ...
    const channelDefs = [
      ...
      { id: 'teams',       label: 'Microsoft Teams', vars: [['TEAMS_APP_ID','App ID'], ['TEAMS_APP_SECRET','App Secret']] },
    ];
...
function cmdLogs() {
  // Try audit.jsonl first (main gateway log)
  const useAuditLog = fs.existsSync(AUDIT_LOG_FILE);
  const logFile = useAuditLog ? AUDIT_LOG_FILE : LOG_FILE;
  ...
  if (!fs.existsSync(logFile)) {
    ...
    console.log(warn(`No log file at ${logFile}`));
    console.log(info('Start the gateway first'));
    return;
  }

  console.log(info(`Tailing ${logFile} (Ctrl-C to exit)...\n`));
...
switch (cmd) {
  case 'onboard':    await cmdOnboard(nonInteractive); break;
